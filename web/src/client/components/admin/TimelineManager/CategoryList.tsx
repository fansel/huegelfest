...
import { FaTrash, FaMusic, FaUsers, FaUtensils, FaCampground, FaGamepad, FaQuestion } from 'react-icons/fa';
        ...
        {categories.map((category) => {
          ...
          >
            <div className="flex items-center gap-2">
                {IconComponent && <IconComponent className="text-[#ff9900]" />}
              <span
                className={`text-sm ${category.isDefault ? 'font-bold' : 'font-medium'}`}
              >
                {category.label}
              </span>
            </div>
