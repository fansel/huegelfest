...
export const AnnouncementCard: React.FC<AnnouncementCardProps> = ({
  ...
  const safeReactions = reactions ?? { counts: {}, userReaction: undefined };
  ...
  return (
    ...
    >
      ...
      {safeReactions && onReact && (
        <div className="flex gap-2 mt-2">
          {Object.keys(REACTION_EMOJIS).map((type) => {
            const reactionType = type as ReactionType;
            const count = safeReactions.counts?.[reactionType] || 0;
            const hasReacted = safeReactions.userReaction === reactionType;
            return (
              <button
                key={reactionType}
                type="button"
