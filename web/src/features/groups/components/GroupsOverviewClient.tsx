...
interface GroupsOverviewClientProps {
  initialData: GroupsData;
}

export function GroupsOverviewClient({ initialData }: GroupsOverviewClientProps) {
  const [activeTab, setActiveTab] = useState<TabType>('groups');
  const [groups, setGroups] = useState<GroupData[]>(initialData.groups);
  const [users, setUsers] = useState<User[]>(initialData.users);
  const [registrations, setRegistrations] = useState<RegistrationWithId[]>(initialData.registrations);
  const [statistics, setStatistics] = useState<GroupAssignmentStats | null>(initialData.statistics);
